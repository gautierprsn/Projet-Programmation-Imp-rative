...
array2D ouverturefichier(int argc, char*argv[]){
  ...
  printf("il y a %i lignes\n",conversion(nombredeligne));
  printf("il y a %i colonnes\n",conversion(nombredecolonne));
  // on a ici le nombre de colonne et de ligne du fichier
...
ensemble differentcas(ensemble e){
  ...
  case 47://correspond au caractère "/", on dépile un entier et on change la direction dans la diagionale supérieure droite si c'est 0, dans la diagonle inférieure gauche sinon.
    a=depile(&e.s);
    if (a==0)
      {e.directory=1;
      }
  ...
  case 34://correspond au caractère " " ", on empile la valeur ASCII suivant en gardant la meme direction jusqu'a atteindre un autre ".
     ...
     while(e.caractere!=34){//cette boucle se termine puisqu'on va croiser un autre "
       //on avance dans la même direction et on affiche les caractères rencontrés.
